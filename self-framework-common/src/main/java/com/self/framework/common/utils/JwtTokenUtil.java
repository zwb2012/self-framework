...
import com.self.framework.common.entity.base.UserInfo;
...
/**
 * description: JwtToken生成的工具类 JWT token的格式：header.payload.signature header的格式（算法、token的类型）： {"alg":
 * "HS512","typ": "JWT"} payload的格式（用户名、创建时间、生成时间）：
 * {"sub":"wang","created":1489079981393,"exp":1489684781} signature的生成算法：
 * HMACSHA512(base64UrlEncode(header) + "." +base64UrlEncode(payload),secret)
 *
 * @author wenbo.zhuang
 * @date 2022/02/17 17:14
 **/
@Slf4j
@SuppressWarnings("all")
public class JwtTokenUtil {

    private static final String CLAIM_KEY_OPEN_ID = "sub";
    ...
    public String getUserIdFromToken(String token) {
        ...
        try {
            ...
            userId = claims == null ? null : claims.getSubject();
        } catch (Exception e) {
            userId = null;
        }
        return userId;
    ...
    public boolean validateToken(String token, UserInfo userInfo) {
        ...
        boolean isUser = StringUtils.endsWith(String.valueOf(userInfo.getId()), id);
        // 判断是否过期,没有设置过期时间调用此方法会报错
    ...
    public String generateToken(UserInfo userInfo) {
        ...
        claims.put(CLAIM_KEY_OPEN_ID, userInfo.getId());
    ...
    /**
     * 判断token在指定时间内是否刚刚刷新过, false：要重新生成token
     *
     * @param token 原token
     * @param time  指定时间（秒）
     */
